...
def input_want_to_create_file_name():
    while True:
        file_name = input(f'作成したい{commands.EXTENSION}ファイル名を入力してください: ')
        if not file_name.endswith(commands.EXTENSION):
            print(f'ファイルの拡張子は{commands.EXTENSION}のみ使用できます')
...
def input_exec_command_file_name():
    [print(f'{file}') for file in get_file_in_dir(commands.DEFAULT_COMMAND_DIR)]
    prompt = 'コマンドファイルのパスを入力してください\n指定したファイルが開けない場合、デフォルトファイルが使用されます: '
    command_file_path = input(prompt)
    command_file_path = commands.DEFAULT_COMMAND_DIR + command_file_path
    if not os.path.isfile(command_file_path):
        command_file_path = commands.DEFAULT_COMMAND_PATH
